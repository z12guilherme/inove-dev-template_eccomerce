...
const Footer = () => {
    ...
    useEffect(() => {
        dbAdapter.getFooterSettings().then(setSettings);

        // Atualiza o rodapé se as configurações forem salvas em outra aba
        const handleStorage = () => {
            dbAdapter.getFooterSettings().then(setSettings);
        };
    ...
    const linkSections = [
        {
            title: "PRODUTOS",
            links: settings.productsLinks.map(l => ({ ...l, icon: null })),
        },
        {
            title: "NAVEGAÇÃO",
            links: settings.navigationLinks.map(l => ({ ...l, icon: null })),
        },
        {
            title: "CONTATO",
            links: [
                { text: settings.contact.phone, path: '/', icon: PhoneIcon },
                { text: settings.contact.email, path: `mailto:${settings.contact.email}`, icon: MailIcon },
                { text: settings.contact.address, path: '/', icon: MapPinIcon },
            ],
        },
    ];
        ...
        <footer className="mx-6 bg-white">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-500/30 text-slate-500">
                    <div>
                        <Link href="/" className="text-4xl font-semibold text-slate-700">
                            <span className="text-green-600">INOVE</span>-DEV<span className="text-green-600 text-5xl leading-0">.</span>
                        </Link>
                        <p className="max-w-[410px] mt-6 text-sm">Bem-vindo à INOVE-DEV, o seu destino definitivo para os gadgets mais recentes e inteligentes. De smartphones a smartwatches e acessórios essenciais, trazemos o melhor da inovação — tudo em um só lugar.</p>
                        <div className="flex items-center gap-3 mt-5">
                            {socialIconsMap.map(({ key, icon: Icon }) => {
                                const href = settings.socialLinks[key];
                                if (!href) return null;
                                return (
                                    <Link href={href} key={key} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-10 h-10 bg-slate-100 hover:scale-105 hover:border border-slate-300 transition rounded-full">
                                        <Icon />
                                    </Link>
                                );
                            })}
                ...
                <p className="py-4 text-sm text-slate-500">
                    Copyright 2025 © INOVE-DEV. Todos os direitos reservados.
                </p>
            </div>
        </footer>
    );
